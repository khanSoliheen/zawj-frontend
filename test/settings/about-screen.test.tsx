...
describe('About screen', () => {
  ...
  it('renders app details and navigates to linked pages', async () => {
    let renderer: TestRenderer.ReactTestRenderer | null = null;

    await act(async () => {
      renderer = TestRenderer.create(<AboutScreen />);
    });
    ...
    expect(texts).toContain('Zawj');
    expect(texts).toContain('1.0.0');
    expect(texts).toContain('12');
    expect(texts).toContain('com.zawj.app');

    act(() => {
      findButtonByLabel(renderer!.root, 'Islamic Policy')?.props.onPress();
    });

    expect(router.push).toHaveBeenCalledWith(ROUTES.SETTINGS_POLICY);
